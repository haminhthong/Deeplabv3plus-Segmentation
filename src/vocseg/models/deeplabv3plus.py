...
def validate_checkpoint_metadata(
    ...
    if arch and arch.lower() != expected_arch.lower():
        raise ValueError(
            f"Checkpoint architecture không tương thích: '{arch}' != kỳ vọng '{expected_arch}'"
        )

    enc = checkpoint.get("encoder")
    if enc and enc.lower() != expected_encoder.lower():
        raise ValueError(
            f"Checkpoint encoder không tương thích: '{enc}' != kỳ vọng '{expected_encoder}'"
        )

    n_cls = checkpoint.get("num_classes")
    if n_cls is not None and int(n_cls) != expected_num_classes:
        raise ValueError(
            f"Checkpoint num_classes không khớp: {n_cls} != kỳ vọng {expected_num_classes}"
        )
